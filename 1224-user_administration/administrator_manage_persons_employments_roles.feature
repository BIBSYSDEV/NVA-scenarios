Feature: Administrator manage Persons, employments, and roles 
	...
	Scenario: Administrator uses the "show more"-option on multiple employments at current Institution
		When the Administrator activate this option (like mouse over)
	...
	Scenario: Administrator views other employments at current Institution
		...
		Then details about each employment is updated
		And other details about the Person and his roles are static

	Scenario: Administrator adds another employment
		When the Administrator adds a new employment
		Then a new affiliation, employment-position and -fraction, start- and end-date is registered
	...
	Scenario: Administrator opens the option to filter the list of Persons
		...
		And all is prechecked, except the "Expired roles"
	...
	Scenario: Administrator wish to add an unknown Person to the Institutional personal register
		...
		Then the Administrator must ask the unknown Person to first log in or import the Person into the personal register

	# Pending Scenario: Administrator do a manual registration of a Person in the personal register
	# we might have to go there...

	Scenario: Administrator adds a new Person to the personal register
